...
import tempfile
import os
from numerology.cycles import calculate_personal_year
...
def build_pinnacle_columns(pinnacles, personal_year_blocks, birthdate):
    ...
    for header_value, block in zip(pinnacles.values(), personal_year_blocks):
        rows = sorted(block.items())
        padded = [
            (str(year), str(calculate_personal_year(birthdate, int(year))))
            for year, _ in rows
        ]
